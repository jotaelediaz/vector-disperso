...
void vectorD<T>::assign(int p, const T & t){
    ...
    typename map<int,T>::iterator it_begin = vd.begin();
    typename map<int,T>::iterator it_end = vd.end();
...
template <typename T >
pair<const int, T > & vectorD< T >::stored_iterator::operator *(  ) {
...
template <typename T >
const pair<const int, T > & vectorD< T >::const_stored_iterator::operator *(  ){
